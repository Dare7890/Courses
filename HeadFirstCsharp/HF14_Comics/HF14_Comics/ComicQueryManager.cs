...
namespace HF14_Comics
{
    class ComicQueryManager
    {
        ...
        private void UpdateAvailableQueries()
        {
            ...
            {
                new ComicQuery("Linq упрощает запросы", "Пример запроса", "Продемонстрируем" +
                "Джимми гибкость LINQ", CreateImageFromAssets("purple_250x250.jpg")),
                new ComicQuery("Дорогие комиксы", "Комиксы дороже 500$.", "Эта штука " +
                "позволит Джимми понять, какие комиксы он хочет больше.", CreateImageFromAssets("captain_amazing_250x250.jpg")),
                ...
                new ComicQuery("Универсальный LINQ 2", "Вычисления в коллекциях",
                    "LINQ предоставляет методы расширения коллекциям (and anything else"
                ...
                new ComicQuery("Универсальный LINQ 3",
                    ...
                    "Иногда результаты запросов нужно сохранять отдельно.",
                    CreateImageFromAssets("bluegray_250x250.jpg")),
                ...
                new ComicQuery("Объединение покупки с ценами",
                    "Посмотрим, умеет ли Джимми торговаться",
                    "Этот запрос создает список классов с приобритениями Джимми",
                    CreateImageFromAssets("captain_amazing_250x250.jpg")),
        ...
        public BitmapImage CreateImageFromAssets(string imageFilename)
        {
            try
            {
        ...
        public void UpdateQueryResults(ComicQuery query)
        {
            ...
            switch (query.Title)
            {
                case "Linq упрощает запросы":
                    LinqMakesQueriesEasy();
                    break;
                case "Дорогие комиксы":
                    ExpensiveComics();
                    break;
                case "Универсальный LINQ 1":
                    LinqIsVersatile1();
                    break;
                case "Универсальный LINQ 2":
                    LinqIsVersatile2();
                    break;
                case "Универсальный LINQ 3":
                    LinqIsVersatile3();
                    break;
                case "Группировка комиксов по диапазону цен":
                    CombineJimmysValuesIntoGroups();
                    break;
                case "Объединение покупки с ценами":
                    JoinPurchasesWithPrices();
                    break;
                case "Все комиксы коллекции":
                    AllComics();
                    break;
            }
        }
        ...
        public IEnumerable<Comic> BuildCatalog()
        {
        ...
        private object CreateAnonymousListViewItem(string title, string filename
            = "bluegray_250x250.jpg")
        {
            return new
            {
                Title = title,
                Image = CreateImageFromAssets(filename),
            };
        }
